...
export function QuantumNebulaHeroVisual({ variant = "panel" }: { variant?: "panel" | "background" }) {
  ...
  return (
    <div
      className={
        isBackground
          ? "absolute inset-0 overflow-hidden bg-[#10081d]"
          : "relative min-h-[430px] overflow-hidden rounded-[8px] border border-plasma/20 bg-[#10081d] shadow-[0_30px_110px_rgba(91,43,214,0.22)] sm:min-h-[540px]"
      }
    >
      ...
      <div
        className={
          isBackground
            ? "pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_52%_42%,transparent_0%,rgba(16,8,29,0.14)_38%,rgba(16,8,29,0.82)_82%),linear-gradient(90deg,rgba(16,8,29,0.76),rgba(16,8,29,0.18)_50%,rgba(16,8,29,0.44)_100%)]"
            : "pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_42%,transparent_0%,rgba(16,8,29,0.08)_42%,rgba(16,8,29,0.72)_100%)]"
        }
      />
      {!isBackground ? (
        <div className="pointer-events-none absolute left-5 top-5 border-l-2 border-t-2 border-plasma/80 pl-4 pt-3">
          <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-bone/52">Quantum assay field</p>
          <p className="mt-1 font-display text-lg font-semibold text-bone">18k live particles</p>
        </div>
      ) : null}
    </div>
  );
